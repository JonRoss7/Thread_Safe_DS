...
void producerConsumerProblem() {
    ...
    auto producer = [&](ThreadSafeQueue<std::string>& queue, std::atomic<int>& produced_count, int id) {
    ...
    auto consumer = [&](ThreadSafeQueue<std::string>& queue, std::atomic<int>& produced_count, int id) {
        int consumed = 0;
    ...
    for (int i = 0; i < NUM_PRODUCERS; ++i) {
        producers.emplace_back(producer, std::ref(messageQueue), std::ref(messages_produced), i + 1);
    }
    for (int i = 0; i < NUM_CONSUMERS; ++i) {
        consumers.emplace_back(consumer, std::ref(messageQueue), std::ref(messages_produced), i + 1);
    }
